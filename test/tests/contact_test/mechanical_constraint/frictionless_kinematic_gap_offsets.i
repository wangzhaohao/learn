...
[AuxKernels]
  ...
  [../]
  [./mapped_primary_gap_offset]
    #返回间隙另一个边界上(paired_boundary)的最近位置处变量值
    type = GapValueAux
...
[Contact]
  [./leftright]
    ...
    model = frictionless
    penalty = 1e+6
    # secondary_gap_offset 和mapped_primary_gap_offset是两个设置两个面的偏移量，也就是这个地方实际上是按照
    # 正常的位置加上偏移量进行计算的间隙
    # 也就是secondary_gap_offset 和mapped_primary_gap_offset都是在secondary上的变量
    secondary_gap_offset = secondary_gap_offset
    # mapped_primary_gap_offset 是通过GapValueAux获得的2上最短距离处的primary_gap_offset， 输出是mapped_primary_gap_offset，存储在boundary=3上
    # 此外需要说明的是，偏移量对正负的固定，间隙缩小方向是负，间隙扩大方向是正
